Kubernetes содержит набор независимых, объединённых вместе процессов управления, которые `постоянно стараются привести текущее состояние контейнеризированного приложения к заданному состоянию` основываясь на декларативном подходе, когда мы описываем требуемое состояние (декларация), а не действия, которые надо предприянть и `неважно, как добраться от А до С, K8s сделает это за вас`.
...
> - labels (лейблы, метки, этикетки) которые используются для идентификации и выбора группы объектов. Рекомендуемые лейблы https://kubernetes.io/ru/docs/concepts/overview/working-with-objects/common-labels/
...
### Helm (штурвал) - управляет установкой, обновлением и настройкой приложений с помощью charts в Kubernetes

Charts (схемы) - помогут вам настроить, установить и обновить даже самое сложное приложение Kubernetes, -  это пакеты Helm, состоящие из файлов и шаблонов YAML, которые при развертывании (deployment) преобразуются в файлы манифестов Kubernetes. 
...
Чарты нужны чтобы было легко обьединить группу всех ресурсов, таких как Deployment, Service, Configmap и прочие, необходимую для работы приложения, версионировать (обновлять), публиковать (как один файл) и делиться ими.
...
2. Установить Jira через helm в свой Kubernetes https://artifacthub.io/packages/helm/atlassian-data-center/jira
   2.1. Найти как из service или из pod пробросить порт и открыть Jira в браузере
   2.2. Посмотреть из каких файлов состоит чарт: https://github.com/atlassian/data-center-helm-charts/tree/main/src/main/charts/jira
