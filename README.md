Kubernetes содержит набор независимых, обьедененных вместе процессов управления, которые `постоянно стараются привести текущее состояние к заданному состоянию` и `неважно, как добраться от А до С, K8s сделает это за вас`
...
Нужны чтобы отделять группы обьектов (контенйнеров и их сетевые или любые другие настройки) в одном кластере
