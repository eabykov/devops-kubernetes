Kubernetes содержит набор независимых, объединённых вместе процессов управления, которые `постоянно стараются привести текущее состояние к заданному состоянию` и `неважно, как добраться от А до С, K8s сделает это за вас`
...
### Helm (штурвал) - управляет установкой, обновлением и настройкой приложений с помощью charts (схем) в Kubernetes
...
3. Для того чтобы k8s успел убрать pod и endpoint для service перед отправкой SIGTERM сигнала необзодимо добавить sleep перед остановкой (который значительно меньше `terminationGracePeriodSeconds`):
  ```yaml
  spec:
    template:
      spec:
        ...
        - lifecycle:
            preStop:
              exec:
                ...
                - "10"
  ```
4. Не забывайте выставлять limits и requests для ваших контенйнеров в pod чтобы k8s использовал requests для плаинрования, а limits для ограничения конкуренции за ресуурсов на node (простой способ как подобрать написанно выше в Deployment) 
5. Настройте HPA для вашего Deployment с запасом 2х от текущей нагрузки (по результатам нагрузочного тестирования)
   > С пиковой дневной нагрузкой за 7 дней справляются 6 pod, выставляем `maxReplicas: 12` (смотрим чтобы не было блокиров когда закончатся подключения к БД и тд)
6. Настройте в вашем service mesh политику повторных запросов при ошибке от нижестоящего сервиса чтобы избежать ошибки в случаях:
