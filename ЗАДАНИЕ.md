# Первое Задание | Реальный пример ресурсов
...
2. Посмотреть какие ресурсы уже есть в кластере командой `watch sudo k3s kubectl get all --all-namespaces` и дождаться когда все они будут готовы
3. Веб интерфейс к кластеру используя `dashboard`:
...
# Второе задание | Мониторинг k8s кластера
...
# Третье Задание | Простой способ установки и обновлений приложений в k8s
...
2. Посмотреть какие есть встроенные переменные в helm: https://github.com/helm/helm-www/blob/main/content/en/docs/chart_template_guide/builtin_objects.md
   - Например проверить есть ли api `autoscaling/v2` для ресурса `HorizontalPodAutoscaler`:
      ...
      ```
# Четвертое задание | Наблюдаемость и управление сетью в k8s с помощью service mesh
...
В итоге вы получите доступ к dashboard где сможете посмотреть на золотые сигналы мониторинга:
